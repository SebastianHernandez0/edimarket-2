...
export function MyOrders() {
    ...
    return (
        <div>
            <div className={classNames('pt-10 bg-white shadow-sm rounded-sm', myOrders.myOrders__container)}>
                ...
                <div className="orders_box">
                    {loading ? (
                        ...
                        orders.map((order, index) => {
                            ...
                            return (
                                <div className={classNames("order_box flex flex-row", myOrders.order__container)} key={`${randomId}_${index}`}>
                                    <div>
                                        <h2 className='pb-2'>Número de órden: #{`${randomId}`}</h2>
                                        <p>Fecha de la compra: {formatDate(order.fecha_venta)}</p>
                                        <p className='font-semibold'>{formatearPrecio(order.valor_total)}</p>
                                    </div>
                                    <div>
